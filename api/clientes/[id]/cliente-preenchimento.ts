  ...
  try {
    ...
    const updated = {
      ...
      identificacao: {
        ...
        ...(identificacao || {}),
        nomeCompleto:
          identificacao?.nomeCompleto || current.identificacao?.nomeCompleto || "",
      },
      queixaPrincipal:
        queixaPrincipal !== undefined ? queixaPrincipal : current.queixaPrincipal,
      historicoSaude: {
      ...
      assinaturaPaciente:
        assinaturaPaciente || current.assinaturaPaciente || "",
      assinaturaPacienteData: assinaturaPaciente
        ? new Date().toISOString()
        : current.assinaturaPacienteData || "",
      status:
        current.status === "aguardando_cliente"
          ? "preenchido_cliente"
          : current.status,
    ...
    // Confirma que a gravação realmente ocorreu antes de responder sucesso.
    const confirmation = await getDoc(
      doc(db, "clientes", currentPatient.documentId),
    );

    if (!confirmation.exists()) {
      throw new Error("O Firestore não confirmou a gravação do formulário.");
    }

    return res.status(200).json({
      ...confirmation.data(),
      id: (confirmation.data() as any)?.id || confirmation.id,
    });
  } catch (error: any) {
    console.error("PUT /api/clientes/[id]/cliente-preenchimento falhou:", error);
    return res.status(500).json({
      error: "Falha ao salvar os dados do paciente.",
